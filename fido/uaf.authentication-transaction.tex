...
\mess[0]{C}{$h$, $ak$, $t$, $fc$ = hash(${\textop{fcp}}$), ...}{ASM}{black,->}
\node [anchor=west] at (mess from) {\shortstack[l]{1. Select authenticator(s) according to $p$\\2. Lookup the key handle ($h$) and access key ($ak$)\\3. ${\textop{fcp}}$ = ($a$, $c$, FACET_ID, TLS_DATA)"}};
\node [anchor=east] at (mess to) {};
\postlevel
...
\node [anchor=east] at (mess from) {};
\node [anchor=west] at (mess to) {\shortstack[l]{1. Unlock UAuth Key Pair = (${\textop{Auth}}.{\textop{pub}}$, ${\textop{Auth}}.{\textop{priv}}$) for this handle $h$ by $ak$\\2. Generate a nonce ($n$)\\3. Generate the Sign Data = ${\textop{SignData}}$ = ($fc$, $n$, ${\textop{cntr}}$, $t$, ${\textop{sig}}$ = signature_by_${\textop{Auth}}.{\textop{priv}}$($fc$, $n$, ${\textop{cntr}}$, $t$))\\${\textop{cntr}}$ = Signature Counter.}};
\postlevel

\mess[0]{A}{${\textop{SignData}}$}{ASM}{black,->}
\node [anchor=east] at (mess from) {};
\node [anchor=west] at (mess to) {};

\mess[0]{ASM}{${\textop{SignData}}$}{C}{black,->}
\node [anchor=east] at (mess from) {};
\node [anchor=west] at (mess to) {};

\mess[0]{C}{${\textop{SignData}}$}{B}{black,->}
\node [anchor=east] at (mess from) {};
\node [anchor=west] at (mess to) {};

\mess[0]{B}{${\textop{SignData}}$}{W}{black,->}
\node [anchor=east] at (mess from) {};
\node [anchor=west] at (mess to) {};

\mess[0]{W}{${\textop{SignData}}$}{S}{black,->}
\node [anchor=east] at (mess from) {};
\node [anchor=west] at (mess to) {1. Verify the ${\textop{SignData}}$ signature by ${\textop{Auth}}.{\textop{pub}}$};
