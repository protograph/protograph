...
\newinst[6]{S}{FIDO Server}
  ...
  \postlevel
	...
	\node [anchor=east] at (mess from) {};
	\node [anchor=west] at (mess to) {\shortstack[l]{1. Generate UAuth Key Pair = ($K.pub$, $K.priv$) for this handle $h$ = ($a$, $u$)\\2. Generate the Key Registration Data = $KRD$ = ($AAID$, $h$, $K.pub$, $fc$, $Att.cert$, $sig$ = signature_by_$Att.priv$($AAID$, $K.pub$, $fc$, $Att.pub$, ...), ...)\\\\$AAID$ = Authenticator Attestation ID\\$Att.cert$ = Authenticator Certificate\\$Att.pub$, $Att.priv$ = Authenticator Key Pair}};
  \postlevel
