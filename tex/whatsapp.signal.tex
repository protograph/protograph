...
\newinst[6]{R}{recipient: $b$
  ...
  \postlevel
	\mess[0]{I}{$Q_{\mathsf{I}_a}, Q_{\mathsf{S}_a}, Q_{\mathsf{O}_a}, \mathsf{SIGN}(\mathsf{I}_a, Q_{\mathsf{S}_a})$}{S}{black,->}
	...
	\node [anchor=west] at (mess to) {};
	\mess[0]{R}{$Q_{\mathsf{I}_b}, Q_{\mathsf{S}_b}, Q_{\mathsf{O}_b}, \mathsf{SIGN}(\mathsf{I}_b, Q_{\mathsf{S}_b})$}{S}{black,->}
	...
	\node [anchor=west] at (mess to) {};
  \postlevel
	\mess[0]{I}{cipher, HMAC($K_\mathsf{message}$, cipher), $Q_{\mathsf{E}_a}$}{R}{black,->}
	\node [anchor=east] at (mess from) {\shortstack[l]{Ephemeral Key: $\mathsf{E}_a = (d_{\mathsf{E}_a}, Q_{\mathsf{E}_a})\gets \mathsf{Curve25519}()$\\$K_\mathsf{message} = \mathsf{HMAC}(K_\mathsf{chain}, 1) $\\$K_\mathsf{chain} = \mathsf{HMAC}(K_\mathsf{chain}, 2) $\\$\mathsf{cipher} = \mathsf{ENCRYPT}(K_\mathsf{message}, \mathsf{message})$}};
	\node [anchor=west] at (mess to) {\shortstack[l]{$K_\mathsf{master} \gets K_1 \Vert  K_2 \Vert  K_3 \Vert  K_4 $\\$K_\mathsf{root}, K_\mathsf{chain} = \mathsf{HKDF}(K_\mathsf{master})$\\$K_\mathsf{message} = \mathsf{HMAC}(K_\mathsf{chain}, 1) $\\$K_\mathsf{chain} = \mathsf{HMAC}(K_\mathsf{chain}, 2) $\\$\mathsf{message} = \mathsf{DECRYPT}(K_\mathsf{message}, \mathsf{cipher})$}};
  \postlevel
	\mess[0]{I}{}{R}{white,->}
